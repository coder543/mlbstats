...
// This test ensures that all special cases are being properly handled.
func TestSortGamesWithPreferredTeam(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Split double header, in progress needs swap",
			args: args{
				teamID: 115,
				games: []Game{
					GenerateGame(1, "N", GameStatusFinal, false, today),
					GenerateGame(115, "S", GameStatusFinal, false, today),
					GenerateGame(115, "S", GameStatusInProgress, false, today.Add(30*time.Minute)),
				},
			},
			want: []Game{
				GenerateGame(115, "S", GameStatusInProgress, false, today.Add(30*time.Minute)),
...
// As I was refactoring the Schedule type into multiple smaller types,
// I wanted to make sure that no data is being unexpectedly lost.
